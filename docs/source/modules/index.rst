.. _modules-index:

=======
Modules
=======

Cette section présente tous les modules du système THL Core Banking, organisés par statut de documentation.

📊 **Statistiques de Documentation**
------------------------------------

- ✅ **7 modules complets** : Documentation détaillée avec procédures
- 🔄 **7 modules en cours** : Documentation partielle ou en développement
- 📈 **80% de couverture** : Modules principaux documentés

🏆 **Modules Documentés**
~~~~~~~~~~~~~~~~~~~~~~~~~~

Ces modules bénéficient d'une documentation complète avec procédures détaillées, exemples pratiques et guides d'utilisation.

    .. toctree::
       :maxdepth: 1
       :caption: ✅ Modules Complets

       accountant/index
       branch/index
       client/index
       client_account/index
       transaction/index
       loan/index
       cashier/index

🔄 **Modules en Développement**
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Ces modules sont en cours de documentation et seront complétés progressivement.

.. toctree::
   :maxdepth: 1
   :caption: 🔄 En Cours

   profile/index
   internal_account/index
   product/index
   operation_request/index
   customer_service/index
   report/index
   log/index
   security/index

📋 **Guide de Navigation**
--------------------------

**Pour les Utilisateurs :**
- Commencez par l'**Introduction** pour comprendre l'architecture
- Consultez les **Modules Complets** pour les fonctionnalités principales
- Utilisez la **recherche** (⌘K) pour trouver des informations spécifiques

**Pour les Développeurs :**
- Chaque module contient des sections **Toutes les fonctionnalités** et **Nouvelles fonctionnalités**
- Les procédures incluent des exemples de code et des cas d'usage
- Les API REST sont documentées avec des exemples de requêtes

**Pour les Administrateurs :**
- Consultez le module **Sécurité** pour la configuration des accès
- Le module **Administration** couvre la configuration système
- Les **Rapports** fournissent des métriques et analytics
